...
public class LELogicDialog extends LogicDialog{
    ...
    public GlobalVarsDialog globalsDialog = new GlobalVarsDialog();
    public boolean wasRows, wasPortrait, forceRestart;
    ...
    public static Color overrideTypeColor(LVar s, Color color) {
        return color.set(
                s.objval instanceof Bullet ? Pal.accentBack :
                        s.objval instanceof LogicSeq ? Pal.place :
                                typeColor(s, color)
        );
    }

    public static String overrideTypeName(LVar s) {
        return s.objval instanceof Bullet ? "bullet" :
                s.objval instanceof LogicSeq ? "array" :
                        typeName(s);
    }
        ...
        buttons.button("@edit", Icon.edit, () -> {
            ...
            dialog.cont.pane(p -> {
                ...
                p.table(Tex.button, t -> {
                    ...
                    if (LEMain.mdtXMode){
                        t.row();
                        t.button(Core.bundle.get("mdtx.logic.discardChanges"), Icon.cancel, style, () -> ui.showConfirm(Core.bundle.get("mdtx.logic.confirmDiscard"), () -> {
                            noSave = true;
                            dialog.hide();
                            hide();
                        })).marginLeft(12f);
            ...
            dialog.cont.pane(p -> {
                ...
                p.table(Tex.button, t -> {
                    ...
                    for(var s : executor.vars){
                        ...
                        t.add(new Image(Tex.whiteui, overrideTypeColor(s, new Color()).mul(mul))).update(i -> i.setColor(overrideTypeColor(s, i.color).mul(mul))).width(stub);

                        t.stack(new Image(Tex.whiteui, overrideTypeColor(s, new Color())){{
                            update(() -> setColor(overrideTypeColor(s, color)));
                        }}, new Label(() -> " " + overrideTypeName(s) + " "){{
